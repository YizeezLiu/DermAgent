...
# Load .env from project root (DermAgent/.env)
from dotenv import load_dotenv
load_dotenv(Path(__file__).resolve().parent.parent / ".env")


def _get_openai_client() -> OpenAI:
    """Create an OpenAI client using env vars (supports proxy / relay APIs)."""
...
def _detect_media_type(img_path: str) -> str:
    """Detect actual image format from file magic bytes, not extension.

    Some datasets have extension mismatches (e.g. JPEG content in .png files).
    Using magic bytes avoids MIME type mismatches that cause API failures.
    """
    with open(img_path, "rb") as f:
        header = f.read(8)
    ...
    mime = {"jpg": "jpeg", "jpeg": "jpeg", "png": "png",
            "gif": "gif", "webp": "webp", "bmp": "png", "tiff": "png"}
    return mime.get(suffix, "jpeg")


def _build_user_content(message: str, img_path: str = None) -> any:
    """Build user message content, optionally including a base64 image."""
    if img_path and os.path.isfile(img_path):
        b64 = _encode_image(img_path)
        media_type = _detect_media_type(img_path)
        return [
...
class Agent:
    ...
    def chat(self, message, img_path=None, chat_mode=True):
        if self.model_info == 'gemini-pro':
            ...
            content = _build_user_content(message, img_path)
...
def process_basic_query(question, examplers, model, args, img_path=None, task_hint=None):
    ...
    # Use task-appropriate prompts for non-MCQ tasks
    if task_hint == "captioning":
        sys_prompt = ('You are an expert dermatologist who provides detailed clinical '
                      'descriptions of skin lesion images.')
        user_prompt = f'{question}'
    else:
        sys_prompt = ('You are a helpful assistant that answers multiple choice questions '
                      'about medical knowledge.')
        user_prompt = (f'The following are multiple choice questions (with answers) about '
                       f'medical knowledge. Let\'s think step by step.\n\n'
                       f'**Question:** {question}\nAnswer: ')

    single_agent = Agent(instruction=sys_prompt, role='medical expert', examplers=new_examplers, model_info=model)
    single_agent.chat(sys_prompt)
    final_decision = single_agent.temp_responses(user_prompt, img_path=img_path)
    
    return final_decision

def process_intermediate_query(question, examplers, model, args, img_path=None, task_hint=None):
    ...
    initial_report = ""
    if task_hint == "captioning":
        opinion_prompt_tpl = (f'Please examine the medical image and provide your expert '
                              f'analysis.\n\n{fewshot_examplers}\n\nQuestion: {question}')
    else:
        opinion_prompt_tpl = (f'Given the examplers, please return your answer to the '
                              f'medical query among the option provided.\n\n'
                              f'{fewshot_examplers}\n\nQuestion: {question}\n\n'
                              f'Your answer should be like below format.\n\nAnswer: ')
    for k, v in agent_dict.items():
        opinion = v.chat(opinion_prompt_tpl, img_path=img_path)
    ...
    if task_hint == "captioning":
        _decision = moderator.temp_responses(f"Given each agent's analysis, please synthesize all expert opinions and provide the final comprehensive description.\n{final_answer}\n\nQuestion: {question}", img_path=img_path)
    else:
    ...
    _moderator_emoji = '\U0001F468\u200D\u2696\uFE0F'
    print(f"{_moderator_emoji} moderator's final decision (by majority vote):", _decision)
    print()

    return final_decision

def process_advanced_query(question, model, args, img_path=None, task_hint=None):
